...
When creating or updating a wiki article, set `volatility` in frontmatter. Default `volatility` to `warm`. The full rubric — what each tier means, when to use it, how the decay differs — lives in `references/wiki-structure.md` § Volatility Classification. The short version: news/trends sources → `hot`, foundational/historical sources → `cold`, everything else → `warm` (the safe default). The author can override during review.

Set `verified` to today's date **only when creating a new article** — the initial synthesis against its sources counts as the first verification. On updates, bump only `updated` and leave `verified` untouched: verification recency must measure when the article's conclusions were last *confirmed*, not when text was last touched, or the freshness score (C14, librarian) degenerates into a clone of compilation recency. After creation, `verified` is updated only by explicit verification flows (human review, `/pkb:refresh`, librarian/audit fixes).
...
5. Update the `updated` date in frontmatter (never `verified` — see the volatility/verified note above)
