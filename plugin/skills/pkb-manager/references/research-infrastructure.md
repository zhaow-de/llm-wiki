...
The Confidence Tag column is the **per-source** input recorded at ingest time.
The final per-article `confidence` is assigned at compile time by the
corroboration rubric in `compilation.md` Step 5 item 8 (e.g. a single score-4 source
yields a `high` source tag but only a `medium` article unless corroborated).
