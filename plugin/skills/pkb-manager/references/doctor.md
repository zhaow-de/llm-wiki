...
## Development Note — The Checkup is the Migration
...
The wiki treats "file in the wrong place from an old version" and "file in the wrong place from user error" as the same defect. `/pkb:doctor --fix` heals both, idempotently. Indexes are already derived caches (see `indexing.md` Derived Index Protocol) — this principle extends to file placement and frontmatter shape.
...
- **Editorial layer (C8/C9)** — project grouping inside `output/projects/`. **Never auto-fixed** because "these files belong together" requires human sense-making. C9 surfaces candidates and emits ready-to-paste `/pkb:project new` + `/pkb:project add` blocks for the user to run.
...
- **Archive lifecycle (C19)** — topic wiki lifecycle under
  `HUB/topics/.archive/`. Archive is quiet preservation: normal checkup reports
...
- **Renamed a `raw/`, `wiki/`, `inventory/`, or `datasets/` directory?** Update the placement map in C11/C16/C17 and the allowlist in C12. Every existing wiki self-heals on the next checkup.
...
There is no `/pkb:migrate` command and there should never be one. Checkup rules **are** the schema.
...
2. Verify `commands/doctor.md` still runs the placement/alias pass in the correct order.
...
- [ ] Hub `topics/.archive/`, when present, contains only archived topic
  ...
  normal topic checkup skips them unless explicitly included.
...
- [ ] Thesis files (`type: thesis`, under `wiki/theses/`) are exempt from the
  article rule above; they require: title, created, updated, tags, summary —
  no `category`, `sources`, or `volatility`, and `confidence` is optional and
  may be `pending` (see `wiki-structure.md` § Thesis File Format)
- [ ] No empty title or summary fields
- [ ] `category` is one of: concept, topic, reference
- [ ] `type` is one of: articles, papers, repos, notes, data (or `thesis` for thesis files)
- [ ] `tags` is a list, not empty
- [ ] `compiled-from`, when present, is one of: sources, conversation, mixed
- [ ] `volatility`, when present, is one of: hot, warm, cold; `confidence`,
  when present, is one of: high, medium, low (on thesis files, also `pending`)
- [ ] Optional collection provenance fields are valid when present — the
  canonical do-not-warn key list lives in C13 below; the value enums live in
  `wiki-structure.md` § Optional Collection Provenance
...
- [ ] "Last compiled" and "Last checkup" dates are present and valid
...
**Execution order**: run C8c (migration) first so migrated projects pass C8a in the same checkup pass. The labels below are in execution order, not alphabetical.
...
Pre-v0.2 wikis have `_project.md` manifests with YAML frontmatter and derived Members sections. When the checkup encounters one:
...
This is the first real application of the checkup-is-the-migration principle codified in this file's dev note. Idempotent — re-running has no effect once WHY.md exists. No separate migration command, no version detection. Just checkup.

### C9: Project Candidates (Critical/Warning/Suggestion)
...
- [ ] **C9b** Any subdirectory inside `output/` that is NOT `projects/` (or `.archive/` inside `projects/`) or `assets/` (media caches written by `/pkb:collect`, e.g. `output/assets/collect-<slug>/`) and contains files — architecture violation, all other subdirectories should be under `output/projects/`. (**Critical**)
- [ ] **C9c** Any `output/projects/<slug>/` folder without a `WHY.md` — this is a malformed project. Suggest: `echo "# <Title>\n\nTODO: goal" > WHY.md` or run `/pkb:project new <slug> "goal"` after archiving the existing folder. (**Warning**)
...
Suggested: bitcoin-quantum-fud (proposed slug)
  ...
  Files:
    ...
    ...
  Create with:
    /pkb:project new bitcoin-quantum-fud "TODO: fill in goal"
    /pkb:project add bitcoin-quantum-fud article-bitcoin-quantum-fud-2026-04-05.md
    ...
```
...
*(C10 is intentionally unassigned — the number was skipped when the catalog
was renumbered and is reserved for a future check.)*
...
Any file that is not in the canonical allowlist for its location is either a user mistake, a stale artifact from an older wiki version, or a legitimate new kind of thing that the schema hasn't caught up to. The checkup surfaces it either way. Like C11, this is scoped to `raw/`, `wiki/`, `inventory/`, `datasets/`, and the wiki root — not `output/projects/` (C8 handles that).
...
> **Note on `.obsidian/` at topic wiki root**: The canonical Obsidian vault now lives at the hub root, so a topic-level `.obsidian/` is redundant — the agentic checkup surfaces it as a Suggestion (it is not auto-removed).
...
**Why this check exists at all (even while empty):** we want the *framework* for schema evolution in place before we need it, so the first rename ever made to a frontmatter field is a one-line addition to a table rather than "let's design a migration system." The dev note at the top of this file explains the full checkup-is-the-migration principle. C13 itself is the mechanism.

**Canonical optional raw-source keys** (do not warn as unknown). This is the
single authoritative list — `wiki-structure.md` § Optional Collection
Provenance and C2 both defer to it:
`collection`, `adapter`, `upstream_id`, `upstream_type`, `revision`, `sha`,
`canonical_url`, `content_format`, `license`, `authors`, `categories`,
`outlinks`, `fetched`, plus the adapter-specific keys
`row_number`, `message_id`, `author`, `date`, `subject`, `dataset_sha`,
`source_columns` (csv-messages), `wayback_timestamp`, `wayback_original`,
`wayback_digest`, `statuscode`, `mimetype`, `length` (wayback-cdx), and
`sha256`, `page_count`, `extraction_tool`, `extraction_status` (PDF
ingestion).
...
- [ ] For fields with known enums (`type`, `category`, `confidence`, `volatility`, `compiled-from`), scan values against the value-alias table. If a match is found, rewrite the value to canonical.
...
**Output**: `Freshness score [score]/100: [article] — source age [avg days], verified [days] ago, compiled [days] ago, [N/M] sources intact. Run /pkb:refresh [path]`
...
Flags wiki articles that lack the `volatility` field. New articles should always have volatility set during compilation. Thesis files (`type: thesis`) are exempt — their lifecycle is governed by `status`, not freshness decay, and they carry no `volatility` field.
...
the checkup should not create a blank inventory tree unless part of the layer already
exists.
...
- Never auto-convert output artifacts into inventory records. Report suggested
  commands such as:
  `/pkb:inventory migrate-output output/ingest-queue-2026-05-03.md --kind ingest-candidate --dry-run`
...
opportunity, not corruption, and the checkup should not create a blank registry unless
...
- Never auto-convert output artifacts, raw data files, or inventory records into
  ...
  `/pkb:dataset migrate-output output/bitcointalk-data-2026-05-03.md --dry-run`
...
- [ ] For each `.md` file in `wiki/` (excluding `_index.md` and thesis files), check that frontmatter has either:
...
- [ ] **Skip thesis files** (`type: thesis`, under `wiki/theses/`): their evidence lives in the body's Evidence For/Against sections, not a `sources:` list, so they are never flagged for missing sources.
...
**Auto-fix**: None. Wiring sources requires reading the article body, identifying its origin raw files, and writing accurate paths — not a default-fillable. Surface the file with a one-line suggestion: `Compiled article <path> has no sources. Recompile the relevant raw source with /pkb:compile --source <raw-source-path>, run /pkb:compile --full, OR add 'compiled-from: conversation' if this article was authored from chat without fetchable sources.`
...
- [ ] `wikis.json` entries with `status: archived` point to an existing
  `topics/.archive/<slug>` directory, or the checkup reports the stale registry entry.
...
- Normal hub checkup reports `Archived topics: N skipped` and does not recursively
  inspect archived topic content.
- Normal topic checkup has no archive behavior unless the target topic path itself
...
- Do not move a topic into or out of archive during checkup. Archive and restore
...
| Unresolved retraction marker | Warn: "Retracted claim not yet reviewed — run `/pkb:retract --recompile` or edit manually" |
| **C8a** `output/projects/<slug>/` missing `WHY.md` | **Report only — never auto-fixed** (severity is Critical). A project without rationale is a malformed project. Report and prompt the user to create one. Auto-creation would manufacture a fake goal, which is worse than the missing file. |
...
| **C9c** Project folder without `WHY.md` | **Warn only** — same as C8a but surfaced in the candidates section. Suggest running `/pkb:project new <slug> "goal"` with the existing slug. |
| **C9d** Loose markdown cluster | **Never auto-fix** — grouping is human-authored via `/pkb:project new` + `/pkb:project add`. |
...
| **C14** Article below freshness score threshold | **Warn/Info only** — composite score below `freshness_threshold` (default 70). Report score breakdown and suggest `/pkb:refresh`. |
...
| **C16** Output looks like inventory | Warn only — suggest `/pkb:inventory migrate-output <path> --dry-run`; never auto-migrate |
...
| **C17** Output looks like a dataset manifest | Warn only — suggest `/pkb:dataset migrate-output <path> --dry-run`; never auto-migrate |
