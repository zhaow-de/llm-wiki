...
│   ├── REPORT.md
│   ├── scan-results.json
│   ├── checkpoint.json            # Resume state for interrupted scans
│   └── log.md                     # Librarian run history
├── .audit/                        # Optional: umbrella audit reports
│   ├── REPORT.md
│   ├── scan-results.json
│   └── log.md                     # Audit run history
├── config.md                      # Title, scope, conventions
├── log.md                         # Topic-level activity log
├── inbox/                         # Drop zone for this topic
│   ├── .processed/
│   └── .unknown/                  # Checkup C12 quarantine for unknown files
...
└── output/                        # Generated artifacts
    ├── _index.md
    ├── assets/                    # Media caches written by /pkb:collect
    │   └── collect-<slug>/        # Downloaded/thumbnailed originals per collection
    ...
    │   └── .archive/              # Archived projects (moved here by /pkb:project archive)
    │       └── <slug>/
    │           └── WHY.md
...
Same structure as above but rooted at `<project>/.llm-wiki-data/` without `wikis.json` or `topics/`. Add `.llm-wiki-data/` to the project's `.gitignore`.
...
1. `--local` flag present → `<cwd>/.llm-wiki-data/`
...
```json
{
  ...
  "local_wikis": [
    { "path": "/absolute/path/.llm-wiki-data", "description": "..." }
  ]
}
```
...
## [2026-04-05] doctor | 19 checks, 0 critical, 2 warnings, 3 suggestions, 1 auto-fixed
...
Operations: `init`, `ingest`, `ingest-collection`, `compile`, `query`, `doctor`, `research`, `thesis`, `collect`, `output`, `refresh`, `retract`, `librarian`, `audit`, `plan`, `project`, `inventory`, `dataset`, `archive`, `ll`, `assess`
...
Raw files created by `/pkb:ingest-collection` may include additional
frontmatter. These keys are canonical and should not be flagged as unknown:
...
content_format: markdown|mediawiki|wikitext|text|csv|tsv|json|jsonl|html|pdf
...
Adapter-specific keys are also canonical when present (see `ingestion.md` for
which adapter writes which): `row_number`, `message_id`, `author`, `date`,
`subject`, `dataset_sha`, `source_columns` (csv-messages);
`wayback_timestamp`, `wayback_original`, `wayback_digest`, `statuscode`,
`mimetype`, `length` (wayback-cdx); `sha256`, `page_count`,
`extraction_tool`, `extraction_status` (PDF ingestion). The full
do-not-warn-as-unknown list lives in `doctor.md` C13 — extend that list (and
this section) together when an adapter adds a key.
...
Required by checkup C2: `title`, `category`, `created`, `updated`, `tags`,
`summary`, plus either `sources` or `compiled-from: conversation`. The
remaining fields (`aliases`, `confidence`, `volatility`, `verified`) are
strongly recommended but optional — C15 nudges missing `volatility` at info
severity, and articles without `verified` are simply not freshness-scored by
C14 (the librarian flags them as `unverified` instead).
...
## Thesis File Format (wiki/theses/)

Thesis files are their own file kind: they use `type: thesis` instead of
`category`, and they track an investigation lifecycle rather than synthesized
knowledge (see `commands/research.md` § Thesis Research Mode for the full
...
created: YYYY-MM-DD
updated: YYYY-MM-DD
verdict: pending|supported|partially-supported|contradicted|insufficient-evidence|mixed
...
tags: [thesis, tag1, tag2]
summary: "One-sentence statement of the claim under investigation"
---
```

Required by checkup C2 for thesis files: `title`, `created`, `updated`, `tags`,
`summary` (no `category`, `sources`, or `volatility` — evidence lives in the
body's Evidence For/Against sections, and staleness is governed by `status`,
not freshness decay).

## Source Reference Resolution

The `sources:` field is a path list, not a bag of slugs. Maintenance workflows
that follow provenance (`librarian`, `doctor`, `audit`, `refresh`, and project
...
Wiki articles carry a `volatility` field that controls how quickly their freshness score decays. The `verified` field records when a human last confirmed the article's conclusions are still accurate. Compilation sets `verified` once, at article **creation** (the initial synthesis against its sources counts as the first verification); recompiles and updates must bump only `updated`, never `verified` — otherwise verification recency would just mirror compilation recency and mask exactly the staleness C14 and the librarian exist to catch. After creation, only explicit verification (human review, `/pkb:refresh`, librarian/audit fix flows) may update `verified`.
...
The freshness threshold is set per wiki in `config.md` (default: 70). Articles scoring below the threshold are flagged by the checkup. There are no hardcoded day cutoffs — the composite score naturally flags the right articles at the right time based on their volatility and the actual state of their sources.
...
The wiki is designed to be opened as an Obsidian vault. On `init`, the hub root gets a `.obsidian/` config directory with minimal settings. Key compatibility notes:
...
- The **hub root** holds the single `.obsidian/` vault; topic wikis are folders inside it and have **no** `.obsidian/` of their own. A `--local` wiki (`.llm-wiki-data/`) is self-contained and keeps its own `.obsidian/`.
- One vault across all topics means Obsidian wikilink names can collide across topics; disambiguate with paths or aliases when needed.
...
---
title: "Output Title"
type: summary|report|study-guide|slides|timeline|glossary|comparison|collection
