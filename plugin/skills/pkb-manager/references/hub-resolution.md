...
The hub path can come from config (most common — iCloud, Dropbox, NAS), a symlink at `~/llm-wiki-data/` pointing elsewhere, or `~/llm-wiki-data/` directly (simple). Earlier versions checked `~/llm-wiki-data/` first, but that fails in sandboxed environments where `~/llm-wiki-data/` isn't an allowed path.
...
7. **If no config exists** → try `$HOME/llm-wiki-data/_index.md`. If it exists, **HUB** = `$HOME/llm-wiki-data`. Done.
...
Most sessions hit steps 1-3 and resolve from config. The `~/llm-wiki-data/` fallback is only for users with no config file.
...
> **Config is authoritative.** If `~/.config/llm-wiki/config.json` exists with a `hub_path` or `resolved_path`, ALL initialization MUST happen at the config path. Prefer `hub_path` for initialization when present. Never create a hub at `~/llm-wiki-data/` when config points elsewhere.

> **Never access `~/llm-wiki-data/` when config exists.** In sandboxed environments, `~/llm-wiki-data/` may not be an allowed path. The config path is the only path the agent should touch.
...
> **Permission-denied is not path-missing.** On macOS/iCloud, `stat` can succeed for `HUB`, `HUB/wikis.json`, and `HUB/topics`, while actual reads or directory listings fail with `Operation not permitted` (`errno=1`). In that case the configured `hub_path` is correct. Stop and tell the user to grant Full Disk Access or iCloud Drive access to the exact app launching the agent, then restart the agent. Do not fall back to `~/llm-wiki-data`, do not use `resolved_path`, and do not suggest moving the wiki just because reads are denied.

## Optional setup: symlink

For users who want the convenience of `~/llm-wiki-data/` without granting sandbox access to their real wiki path, a symlink works:

```bash
ln -s "/Users/jane/Library/Mobile Documents/com~apple~CloudDocs/llm-wiki-data" ~/llm-wiki-data
```
...
```bash
hub_path="~/Library/Mobile Documents/com~apple~CloudDocs/llm-wiki-data"  # from config
expanded="${hub_path/#\~/$HOME}"
# Result: /Users/jane/Library/Mobile Documents/com~apple~CloudDocs/llm-wiki-data
...
1. `--local` flag → `.llm-wiki-data/` in current directory
