...
8. **Goal is mandatory at creation.** Enforced by `/pkb:project new <slug> "goal"` — the goal becomes the body of `WHY.md`.
...
The folder is preserved, all files stay put, git history continues. `/pkb:project list` shows active projects only by default; `list --archived` includes `.archive/`.

Reason: a `status: archived` frontmatter field in the old model required the manifest to exist, which required the manifest format, which required the derived-members machinery. Moving the folder is one operation and needs zero schema. The tradeoff is that links to archived projects from other files break — checkup C4 catches those from wiki articles and inventory records; links from other *output* artifacts are outside the checkup's scope (C12 skips `output/`) and surface only during an audit pass.
...
A project is **stale** when new information relevant to it has been ingested since its artifacts were last updated. This is detected by checkup check C8b, not by a manifest field, and the chain runs through frontmatter that already exists:
...
Existing wikis created under v0.1.0/v0.1.1 will have `_project.md` manifests. Checkup check C8c detects these and auto-migrates:
...
6. Report the migration in the checkup output

This is the first real application of the checkup-is-the-migration principle codified in `doctor.md`. One-time healing; idempotent; no separate migration command needed. See `doctor.md` C8c for the full rule.
