...
|----------|------------------|
| `pkb status` | Show active stats and `Archived topics: N`; list archived only with `--archived` |
...
| `doctor` | Active structural checks by default; `--include-archived` and `--archived-only` maintain archived structure explicitly |

## Checkup Semantics

The checkup is responsible for keeping archive structure coherent, but normal checkup
should not resurrect archived maintenance debt.

- Default hub checkup validates the active registry and reports archived topic
  counts as skipped.
- `doctor --include-archived` checks archived topic structure in addition to
  active material.
- `doctor --archived-only` checks only archived topic wikis.
- `doctor --fix --include-archived` may perform mechanical fixes on archived
...
reported by the checkup as a boundary-crossing provenance warning. Do not break the
source chain just because lifecycle state changed.

## Registry Repair

`wikis.json` is the lifecycle source of truth, but the checkup should repair common
filesystem drift:
...
Do not move project-local `.llm-wiki-data/` directories in v1. A local wiki can still
