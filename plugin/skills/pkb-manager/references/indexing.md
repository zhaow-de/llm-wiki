...
Before using any `_index.md`, check staleness. The count basis depends on the
index kind:

- **Leaf content directories** (`raw/<type>/`, `wiki/<category>/`,
  `inventory/<kind>/`, `datasets/<slug>/` and its `samples/`/`profiles/`/`queries/`):
  count loose `.md` files in the directory (excluding `_index.md`) vs rows in
  the contents table.
- **Aggregate indexes** (`raw/_index.md`, `wiki/_index.md`,
  `inventory/_index.md`, `datasets/_index.md`): these list subdirectory
  indexes or per-subdirectory records, not loose files. Count the entries they
  aggregate (sub-indexes, manifests, or records across subdirectories) vs
  their rows — never the loose-file count, which is legitimately zero there.
- **Master `_index.md`**: navigation plus statistics; stale-check its counts
  against the per-layer totals, not against files in the wiki root
  (`config.md` and `log.md` are never Contents rows).

1. Compute the count on the basis above
...
1. List the entries on the same count basis as above (loose `.md` files for
   leaf directories, aggregated sub-entries for aggregate indexes)
2. Read each entry's YAML frontmatter (title, summary, tags, updated)
3. Regenerate the `_index.md` contents table from frontmatter — preserve the
   hand-shaped navigation sections of aggregate and master indexes; only the
   contents/statistics rows are derived
