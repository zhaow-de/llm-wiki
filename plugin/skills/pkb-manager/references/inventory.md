...
The checkup validates them as provenance for tracking state, not as evidence for factual
claims.
...
The checkup should treat missing `inventory/` as a migration opportunity for older
wikis, not as corruption:

- Missing `inventory/` on an existing wiki: suggestion, not critical.
- `doctor --fix`: may repair indexes for an inventory layer that already exists,
...
- The checkup must never auto-convert output artifacts into inventory records.
...
- `doctor`: repairs indexes for an inventory layer that already exists and
