...
Content-level wiki maintenance: staleness detection, quality scoring, factual verification, semantic coherence, deduplication. This reference defines the scoring algorithms, report formats, and operational protocols for `/pkb:librarian`.
...
All librarian passes operate on synthesized wiki articles only. Thesis files
(`type: thesis`, under `wiki/theses/`) are excluded from staleness and quality
scoring: their lifecycle is governed by `status`, not freshness decay, so the
`unverified` / `stale` / `single-source` flags do not apply to them. The scan's
article list is built with thesis files already filtered out.
...
|------------|-----------------|--------|
| `hot` | 30 | Score decays quickly — 30-day-old hot article scores ~50%, 60-day-old ~25% |
...
Articles whose evidence is the conversation that authored them — not fetchable raw files — set `compiled-from: conversation` in frontmatter (see `wiki-structure.md` schema and `doctor.md` C18). For these articles:
...
Four dimensions, each scored 1-5. Composite quality score is the average scaled ×20 (range 20-100; see the formula below).
...
The `.librarian/` directory lives inside each topic wiki (e.g., `~/llm-wiki-data/topics/meta-llm-wiki/.librarian/`). Created on first scan.

### checkpoint.json

Written after each article is scored. If the session drops, `scan --resume` reads this file and skips already-scored articles without prompting; a bare `scan` that finds a checkpoint prompts before resuming (see the command spec's Scan Protocol step 0.2).
...
|---------|---------------|-------------------|
| `doctor` | Structure: broken links, missing indexes, frontmatter schema, file placement | Checkup's territory — librarian skips |
| `doctor --deep` (C7) | Quick spot-check: a few web searches for obvious staleness | Lightweight — librarian goes deeper |
