...
Collect is not the same as `/pkb:research` or `/pkb:ingest-collection`:

- Use `/pkb:research` when the user wants an answer, explanation, thesis, or
  synthesized wiki articles.
- Use `/pkb:ingest-collection` when the input is a structured upstream corpus
  such as a Git repo, MediaWiki dump/API, message archive, or Wayback CDX set.
- Use `/pkb:inventory` directly when the user already knows the records to
  track and no discovery/catalog pass is needed.
- Use `/pkb:collect` when the user says "find", "collect", "catalog",
...
1. Read `$HOME/.config/llm-wiki/config.json`. If it has `hub_path`, expand
   ...
   Access/iCloud Drive access to the launcher; do not fall back to `~/llm-wiki-data` or
   `resolved_path`. Do not write machine-specific `resolved_path` into shared
   configs.
2. If no config exists, read `$HOME/llm-wiki-data/_index.md`. If it exists, use
   `$HOME/llm-wiki-data` as HUB. If nothing is found, ask where to create the wiki.
3. Wiki location, first match: `--local` -> `.llm-wiki-data/`; `--wiki <name>` ->
   `HUB/wikis.json` lookup with portable path resolution (`<HUB>`, `~`,
   absolute, or HUB-relative), falling back to `HUB/topics/<name>` if stale;
   current directory has `.llm-wiki-data/` -> use it; otherwise use HUB.
...
Read `skills/pkb-manager/references/inventory.md` and
`skills/pkb-manager/references/research-infrastructure.md` before writing. Use
