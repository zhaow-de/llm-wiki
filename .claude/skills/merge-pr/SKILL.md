...
5. **No failing CI** — any `statusCheckRollup` entry in a failing state stops the merge. Every PR to `develop`/`main` reports a **required** `Structural validation` check (`.github/workflows/plugin-tests.yml`): it runs the structural tests when `plugin/**`, `tests/**`, or `pkb.py` changed (a failure is a hard stop), and reports a **skipped — passing —** check otherwise. Because the check is required by branch protection, a pending or failing one also surfaces as `BLOCKED` in gate 3. The deterministic merge gate remains the local structural test suite, run before the PR was opened.
...
**If any gate fails:** report which one and why, ask the user to resolve it manually (update the branch, fix the failure, get the review, check the boxes), then **STOP**. Do not merge.
