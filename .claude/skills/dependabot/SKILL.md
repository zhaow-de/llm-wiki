---
name: dependabot
description: Manage Dependabot dependency-update PRs — list, check out, rebase onto develop, run the pre-commit + structural test gate, auto-fix routine issues, push, wait for CI, merge with squash
disable-model-invocation: true
model: haiku
allowed-tools: Bash(git fetch:*), Bash(git checkout:*), Bash(git rebase:*), Bash(git status:*), Bash(git stash:*), Bash(git push:*), Bash(git add:*), Bash(git commit:*), Bash(git log:*), Bash(git branch:*), Bash(gh pr:*), Bash(uv:*), Bash(python3:*), Bash(sleep:*), Bash(./tests/test-plugin-validate.sh:*), Bash(./tests/test-structure.sh:*), Bash(./tests/test-local-cli-doctor.sh:*), Read, Glob, Grep, Edit, Write, AskUserQuestion
---
...
- **This is a Claude Code plugin, not a Python application.** `uv` is present only to run pre-commit and pin dev tooling — there is **no pytest or ruff gate**. The local gate is the pre-commit hooks plus the deterministic structural test scripts (`./tests/test-*.sh`).
- Pre-commit hooks (`.pre-commit-config.yaml` at repo root) auto-format on every `git commit` (mdformat, end-of-file-fixer, trailing-whitespace, etc.). A push after a hook-driven amend may need re-staging — the loop handles it.
- Configured ecosystems: `uv` (updates `pyproject.toml` + `uv.lock` — dev tooling such as pre-commit), `github-actions` (updates `.github/workflows/*` if any are installed), and `pre-commit` (updates `.pre-commit-config.yaml` hook revisions). This skill processes any `dependabot/` PR regardless of ecosystem.
...
(There is no pytest/ruff gate — pre-commit plus the structural test scripts are the full local gate.)

#### 2c. Auto-fix (if validation fails)

| Failure | Auto-action | Max attempts |
|---|---|---|
| A pre-commit hook **rewrote files** (mdformat, end-of-file-fixer, trailing-whitespace) | re-stage with `git add -A` and re-run `uv run pre-commit run --all-files` until clean | 2 |
| A pre-commit hook **reports a non-auto-fixable error** (check-yaml, check-toml, large-files) | read the error; patch the offending file if the fix is obvious | 2 |
| A structural test fails | read the failure; if it is an obvious upgrade-shaped issue (a hook rev changed formatting expectations, a workflow action input changed), patch it; re-run that test | 3 |
...
# The CI workflow (.github/workflows/plugin-tests.yml) runs on every PR, but only
# runs the structural tests when plugin/**, tests/**, or pkb.py changed. Dependabot
# PRs touch dependency files outside those paths, so the required Structural
# validation check reports as SKIPPED — handled as a pass below — and the local
# pre-commit + structural-test gate above is the real gate.
deadline=$(( $(date +%s) + 600 ))
state="pending"
...
4. **CI failures unrelated to the PR's changes** (e.g. infra flake, a structural test that was passing on develop before this branch was opened).
...
- End every commit with a `Co-Authored-By:` trailer naming the **actual** executing model (this skill runs on Haiku — e.g. `Co-Authored-By: Claude Haiku 4.5 <noreply@anthropic.com>`). Always reflect the real model, never a stale example.
